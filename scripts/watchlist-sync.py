...
# --- HTTP helpers ---
_SOCKS_PROXY_HOST = None
_SOCKS_PROXY_PORT = None


def _setup_socks_proxy():
    """Parse SOCKS proxy config (don't set globally)."""
    global _SOCKS_PROXY_HOST, _SOCKS_PROXY_PORT
    if not TORRENTIO_PROXY:
        return
    m = re.match(r"socks5h?://([^:]+):(\d+)", TORRENTIO_PROXY)
    if m:
        _SOCKS_PROXY_HOST = m.group(1)
        _SOCKS_PROXY_PORT = int(m.group(2))
        log(f"SOCKS proxy configured: {_SOCKS_PROXY_HOST}:{_SOCKS_PROXY_PORT}")
...
def http_get(url, headers=None, use_proxy=False, timeout=30):
    ...
    if use_proxy and _SOCKS_PROXY_HOST:
        import socks
        import socket
        orig_socket = socket.socket
        try:
            socks.set_default_proxy(socks.SOCKS5, _SOCKS_PROXY_HOST, _SOCKS_PROXY_PORT, rdns=True)
...
def find_best_torrent(imdb_id, media_type, season=None, episode=None):
...
def rd_add_magnet(info_hash):
    """Add a magnet to RD. Returns torrent ID or None."""
...
def process_watchlist_item(item, state):
    ...
    if media_type == "movie":
        best = find_best_torrent(imdb_id, "movie")
        if not best:
        ...
        best = find_best_torrent(imdb_id, "show")
        if not best:
            log(f"SKIP {title} — no streams found")
            return False
...
def main_loop():
    ...
    _setup_socks_proxy()
    discord("🚀 **Watchlist Sync** started")
